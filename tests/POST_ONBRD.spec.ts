...
test('test @Regression', async ({ page, context }) => {
  ...
  const actualLoginText = await page.locator('div.w-action-item-label-value').textContent();
  const expectedLoginText = 'Tom Tamer';
  await expect(page.locator('div.w-action-item-label-value')).toHaveText(expectedLoginText);
  ...
  await page.locator('sct-key-value div').filter({ hasText: 'Active' }).nth(3);
