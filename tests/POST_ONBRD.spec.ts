...
const lockFilePath = path.join(__dirname, 'test-run.lock');
 
// Utility function to generate unique values

const generateUniqueValue = (prefix = '') => `${prefix}${Date.now()}${Math.floor(Math.random() * 1000)}`;
 
// Create a custom test wrapper that skips if lock file exists

test('Run only once', async ({ page, context }) => {
  ...
  if (fs.existsSync(lockFilePath)) {
    ...
    test.skip(true, 'Test has already run. Skipping...');

    return;

  }
 
  // Create a lock file to mark the test has run

  fs.writeFileSync(lockFilePath, 'Test executed at: ' + new Date().toISOString());
  ...
  page.getByRole('button', { name: 'Logged in Tom Tamer ' });
  ...
  page.locator('#orderDetailsOrderStatus').getByText('Completed');
